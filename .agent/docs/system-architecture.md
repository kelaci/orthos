...
This document provides a high-density overview of the GAIA architecture, optimized for LLM context windows and structural reasoning.

## 📦 Module Dependency Graph

```mermaid
graph TD
    CORE(gaia.core) --> LAYERS(gaia.layers)
    CORE --> HIERARCHY(gaia.hierarchy)
    CORE --> PLASTICITY(gaia.plasticity)
    
    LAYERS --> HIERARCHY
    PLASTICITY --> LAYERS
    
    META(gaia.meta_learning) --> PLASTICITY
    HIERARCHY --> MANAGER(HierarchyManager)
    
    CONFIG(gaia.config) --> ALL(All Modules)
```

## 🧩 Core Components Deep-Dive

### 1. `gaia.core.base.BaseLayer`
...
### 2. `gaia.hierarchy.level.HierarchicalLevel`
...
### 3. `gaia.layers.hebbian.HebbianCore`
...
The standard data flow in GAIA follows this pattern:
...
1.  **Deterministic Initialization**: Always use the seed from `gaia.config.defaults` if available.
...
If you need to find:
- **Math for Oja's rule**: Look in `gaia/plasticity/rules.py`.
- **How levels are added**: Look in `gaia/hierarchy/manager.py`.
- **Logging logic**: Look in `gaia/utils/logging.py`.
- **Default Hyperparameters**: Look in `gaia/config/defaults.py`.
